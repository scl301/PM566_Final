---
title: "Understanding Factors Driving Variation in Medicare Inpatient Hospitalization Rates and Payments by Region and Diagnostic Category"
...
```{r}
# create a new column with the average Medicare payment per state and MDC code
avg_payment_by_state <- cms %>%
  group_by(Rndrng_Prvdr_Geo_Desc, MDC, MDC_Desc) %>%
...
p <- ggplot(avg_payment_by_state, aes(x = reorder(Rndrng_Prvdr_Geo_Desc, desc(avg_payment)), y = avg_payment, fill = MDC, text = paste("MDC Code: ", MDC, "<br>MDC Desc: ", MDC_Desc, "<br>Average Payment: $", round(avg_payment, 2)))) +
  geom_bar(stat = "identity",  width = 0.7) +
  labs(title = "Average Medicare Payment by State and MDC Code",
       x = "State",
...
# create a new column with abbreviated MDC Descriptions and Codes
cms$MDC_Desc <- str_replace(cms$MDC_Desc, "Diseases and Disorders of the ", "")
cms$MDC_Desc <- paste(cms$MDC, cms$MDC_Desc, sep = " - ")

# create the boxplot
p <- ggplot(cms, aes(x = MDC, y = Avg_Mdcr_Pymt_Amt, fill = MDC_Desc)) +
  geom_boxplot() +
  xlab("MDC Code") +
...
ggplotly(p, tooltip = c("MDC_Desc", "y", "lower", "upper", "middle", "lowerf", "upperf"))
...
p <- ggplot(top_mdc, aes(x = reorder(MDC_Desc, desc(total_discharges)), y = total_discharges, fill = Region, 
                          text = paste("Region: ", Region, "<br>MDC Desc: ", MDC_Desc, "<br>Total Discharges: ", total_discharges))) +
